...
3. Выдели отдельные продукты/блюда в items. Для каждого:
   - name: название продукта НА АНГЛИЙСКОМ, максимально конкретное и простое ("chicken breast cooked", "white rice boiled", "buckwheat groats cooked", "apple raw")
   - grams: примерный вес в граммах (если порция не указана — оцени консервативно: стандартная порция)
   - grams_confidence: "high" если граммы явно указаны пользователем, "medium" если можно оценить по контексту, "low" если совсем непонятно
